...
public class Update {
    ...
    public void install() {
        try {
            if (plugin.getConfig().getBoolean("internet")) {
                try {
                    ...
                    if (download(path)) {
                        ...
                        plugin.getLoggerUtility().log("BookShop will be updated on the next restart!", LoggerUtility.Level.WARNING);
                    } else {
                    ...
                    plugin.getReportHandler().report(3313, "Error on downloading new Version", e.getMessage(), "BookShop", e);
                    e.printStackTrace();
            ...
            try {
                plugin.getPluginManager().unloadPlugin("BookShop");
            ...
            try {
                plugin.getPluginManager().loadPlugin("BookShop");
...
//                plugin.getReportHandler().report(3311, "Error checking for new version", e.getMessage(), "BookShop", e);
//                plugin.getLoggerUtility().log("May the mainserver is down!", LoggerUtility.Level.ERROR);
//                plugin.getLoggerUtility().log("Uncatched Exeption!", LoggerUtility.Level.ERROR);
//            }
//        }
//    }
    ...
    public String getLink() {
        String link = "";
        try {
            URL rss = new URL("http://dev.bukkit.org/server-mods/bookShop/files.rss");
